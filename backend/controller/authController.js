  ...
  try {
     ...
     if(!data.email){
      throw new Error("Invalid Link")
       ...
       const updatedUser =  await UserModel.findOneAndUpdate({email:data.email},{
         isVerfied:true
        },
        {
          new:true,
...
const sendConfirmationEmail=async(req,res,next)=>{
  ...
  try {
    ...
    res.status(200).json({message:"successfully resent confimation link",success:true})
...
module.exports = { registerUser, loginUser, logoutUser ,verifyConfirmationEmailHash ,confirmEmailHash ,verifyUser ,sendConfirmationEmail};
