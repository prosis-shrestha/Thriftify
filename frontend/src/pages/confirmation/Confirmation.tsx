...
const Confirmation = () => {
  ...
  const handleCheckIfHasIsValid = async (code: string) => {
    ...
    setLoading(true);
    try {
      const { data, status } = await verifyIfEmailConfirmationTokenIsValidApi(
        code
      );
      if (status === 200) {
        console.log(data);
      ...
      <div className={styles.confirmation_box}>
        ...
        {isExpired || invalidLink ? (
          <button
            ...
            onClick={() => navigate("/account/verify_email")}
          >
            Verify Now.
          </button>
        ) : (
          <button className={styles.confirmBtn} onClick={handleConfirmAccount}>
            {loading ? "CONFIRMING" : "CONFIRM NOW"}
          </button>
        )}
        {isExpired ? (
          <p className={styles.warning}>
            your email confirmation link has been expired .{" "}
          </p>
        ) : (
          ""
        )}
        {invalidLink ? (
          <p className={styles.warning}>your confirmatin link is invalid . </p>
        ) : (
          ""
        )}
      </div>
